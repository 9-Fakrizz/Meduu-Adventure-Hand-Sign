...
int Unit::burse(Unit &opp){
    int hh = rand()%21 + 10;
	if(exp>=5){
	hp = hp - hh;
	exp -=5;}
	return opp.beAttacked(atk*4),hh;
	}

int Unit::ultraheal(){
    int hhh = rand()%21 + 40;
	if(exp>=5){
    if(hp + hhh > hpmax) hhh = hpmax - hp;
	hp = hp + hhh;
    atk -=10;
    exp -=5;
	return hhh;
...
string skill(char A){
    string skl = ""; 
    if(A == '1') {cout << "choose life drain\n";
    skl = "lifedrain";
    return skl;}
	if(A == '2') {cout << "choose buff\n";
    skl = "buff";
    return skl;}
    if(A == '3') {cout << "choose burse\n";
    skl = "burse";
    return skl;}
    if(A == '4') {cout << "choose ultraheal\n";
    skl = "ultraheal";
    return skl;}
